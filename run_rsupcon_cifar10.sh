...
ENCODER_OPT="--batch_size 128 --weight_decay 5e-4 --learning_rate 0.1 --temp 0.07"
LOG_NAME="from_scratch_test"
...
MODEL_CKPT="./ckpt/cifar10/"$LOG_NAME"/last.pth"

LINEAR_AUG="trivial"
LINEAR_LOSS="CE"  # CE, Madry, TRADES
LINEAR_BATCH="512"
LINEAR_EPOCH="10"
LINEAR_LR="1.0"
...
LINEAR_CKPT="./ckpt/cifar10/"$LOG_NAME"/linear_"$LINEAR_LOSS"_"$LINEAR_AUG"_lr_"$LINEAR_LR"_epoch_"$LINEAR_EPOCH"_bsz_"$LINEAR_BATCH".pth"

# # Robustness evaluation across all metrics
EVAL_OPT="--test_option all"
# EVAL_OPT="--test_option partial --test_type adversarial"
python main_eval.py --test_option all --model_ckpt $MODEL_CKPT --linear_ckpt $LINEAR_CKPT --log_name $LOG_NAME

# # Visualization specific to CIFAR-10
python main_vis.py --model_ckpt $MODEL_CKPT  --linear_ckpt $LINEAR_CKPT --log_name $LOG_NAME
